...
import { createVacancy } from "@/actions/vacancy.actions";
    ...
    const handleSave = async () => {
        ...
        try {
            await createVacancy({
                title: roleTitle,
                level: level,
                competencyIds: [], // MVP: skipping explicit Competency table linking
                planBlocks: blocks.map(b => ({
                    title: b.title,
                    goal: b.goal,
                    required: b.required,
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-6 md:p-8 shadow-sm flex flex-col gap-6">
                ...
                <div className="pt-2 flex justify-end">
                    <button
                        ...
                        className={`flex items-center justify-center gap-2 rounded-xl px-6 py-2.5 text-sm font-medium text-white shadow-sm transition-all focus:ring-4 focus:ring-violet-100 ${isGenerating ? 'bg-violet-400 cursor-not-allowed' : 'bg-violet-600 hover:bg-violet-700'
                            }`}
                    >
                        {isGenerating ? (
                            <>
                ...
                <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    ...
                    <div className="mt-8 pt-6 border-t border-zinc-200 flex justify-between items-center sticky bottom-6 bg-white/90 backdrop-blur p-4 rounded-2xl shadow-lg ring-1 ring-zinc-900/5">
                        ...
                        <div className="flex gap-4">
                            ...
                            <button disabled={isSaving} onClick={handleSave} className="rounded-xl bg-zinc-900 px-8 py-2.5 text-sm font-medium text-white shadow-sm hover:focus:ring-4 hover:ring-zinc-200 hover:bg-black transition-all">
                                {isSaving ? "Сохранение..." : "Утвердить и Сохранить Вакансию"}
                            </button>
                        </div>
                    </div>
                </div>
