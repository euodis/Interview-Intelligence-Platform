...
import { mockCandidates, mockSummaries, mockPlans, mockSessions, mockBlockEvaluations } from "@/data/mocks";

export default function CompareCandidatesPage({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const vacancyId = resolvedParams.id;

  // Only candidates that match the vacancy and have some evaluations (status ОЦЕНЕН)
  const availableCandidates = mockCandidates.filter(c => c.vacancyId === vacancyId && c.status === 'ОЦЕНЕН');
  ...
  const plan = mockPlans.find(p => p.vacancyId === vacancyId);
  ...
  const comparisonData = selectedIds.map(id => {
      ...
      const summary = mockSummaries.find(s => s.candidateId === id);
      const sessions = mockSessions.filter(s => s.candidateId === id && s.status === 'ЗАВЕРШЕНО');
      
      // Compute avg scores per block
      const blockScores: Record<string, number | null> = {};
      plan?.blocks.forEach(block => {
          const evals = mockBlockEvaluations.filter(e => e.blockId === block.id && sessions.some(s => s.id === e.sessionId));
          if (evals.length > 0) {
              const avg = evals.reduce((acc, curr) => acc + curr.score, 0) / evals.length;
