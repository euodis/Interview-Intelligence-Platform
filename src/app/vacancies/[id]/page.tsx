...
// Mock Last Update dates for UI realism
...
export default function VacancyDetails({ params }: { params: Promise<{ id: string }> }) {
  const resolvedParams = use(params);
  const vacancy = mockVacancies.find(v => v.id === resolvedParams.id) || mockVacancies[0];
  
  // Pipeline Data
  const candidates = mockCandidates.filter(c => c.vacancyId === vacancy.id);
  const completedSessions = mockSessions.filter(s => candidates.some(c => c.id === s.candidateId) && s.status === 'ЗАВЕРШЕНО');
  
  const summaries = mockSummaries.filter(s => candidates.some(c => c.id === s.candidateId));
  ...
  let tableData = candidates.map(candidate => {
    const summary = mockSummaries.find(s => s.candidateId === candidate.id);
    const candidateSessions = mockSessions.filter(s => s.candidateId === candidate.id);
    const assignedInterviewers = candidateSessions.map(s => mockInterviewers.find(i => i.id === s.interviewerId)).filter(Boolean);
    ...
    return {
      ...
      lastUpdated: mockDates[candidate.id] || '-',
    };
  });
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
         <div className="text-sm border-b border-zinc-200 pb-2">
           <Link href="/" className="text-zinc-500 hover:text-zinc-900 border-b border-zinc-500 pb-0.5">Все Вакансии</Link>{' '} / {vacancy.id}
         </div>
      ...
      <div className="rounded-2xl border-l-[3px] border-violet-500 border border-zinc-200 bg-white p-6 shadow-sm flex flex-col gap-3">
         ...
         <p className="text-sm text-zinc-700 leading-relaxed max-w-4xl">
            У нас 5 активных кандидатов. Выделен <span className="font-semibold">один сильный фаворит (Алексей Смирнов)</span>, готовый к офферу. 
            По одному из кандидатов (Марина Кравец) обнаружены **столкновения в оценках** технических экспертов — требуется синхронизация. 
            В целом, пайплайн содержит кандидатов хорошего уровня, средний балл выше нормы.
         </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
         ...
         <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
            ...
            <p className="text-3xl font-bold text-zinc-900 mt-2">{avgScore} <span className="text-base font-medium text-zinc-400">/ 5</span></p>
         </div>
