...
export default function InterviewProtocolClient({ session, readOnly = false }: { session: any; readOnly?: boolean }) {
    ...
    const canSubmit = !readOnly && completeRequired && confidence !== null && overallNotes.trim().length > 10;
        ...
        <div className="flex flex-col max-w-4xl mx-auto w-full pb-24">
            ...
            {readOnly && (
                <div className="mb-6 rounded-2xl border border-amber-200 bg-amber-50 p-4 flex items-center gap-3">
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="text-amber-600 shrink-0"><rect x="3" y="11" width="18" height="11" rx="2" ry="2" /><path d="M7 11V7a5 5 0 0 1 10 0v4" /></svg>
                    <div>
                        <p className="text-sm font-semibold text-amber-900">Режим просмотра</p>
                        <p className="text-xs text-amber-700">{session.status === 'ЗАВЕРШЕНО' ? 'Протокол уже отправлен. Редактирование недоступно.' : 'Только назначенный интервьюер может заполнять протокол.'}</p>
                    </div>
                </div>
            )}
            ...
            <div className="flex items-center gap-4 mb-6">
                <Link href={readOnly ? `/candidates/${candidate?.id}` : '/interviews'} className="flex items-center justify-center w-10 h-10 rounded-full border border-zinc-200 bg-white hover:bg-zinc-50 transition-colors">
                    &larr;
                </Link>
                <div>
                    <h1 className="text-2xl font-bold tracking-tight text-zinc-900">Интервью: {candidate?.name}</h1>
                    <p className="text-sm font-medium text-zinc-500">{vacancy?.title} • {readOnly ? 'Просмотр протокола' : 'Session Setup'}</p>
                </div>
                {!readOnly && (
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                ...
                {plan.blocks.map((block: any, i: number) => {
                    ...
                    return (
                        <div key={block.id} className={`rounded-2xl border bg-white shadow-sm overflow-hidden transition-colors ${state.score ? 'border-zinc-200' : block.required ? 'border-violet-200 shadow-violet-50' : 'border-zinc-200'
                            }`}>
                                ...
                                <div className="flex items-start justify-between">
                                    <div>
                                        <div className="flex items-center gap-2 mb-1">
                                            ...
                                            {block.required && <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-violet-100 text-violet-800">Обязательно</span>}
                                        </div>
                            ...
                            <div className="p-6 flex flex-col gap-6">
                                        ...
                                        <div className="flex items-center gap-2">
                                            {[1, 2, 3, 4, 5].map(num => (
                                                <button
                                                    ...
                                                    type="button"
                                                    disabled={readOnly}
                                                    onClick={() => !readOnly && updateBlock(block.id, 'score', num)}
                                                    onDoubleClick={() => !readOnly && updateBlock(block.id, 'score', null)}
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        value={state.notes}
                                        readOnly={readOnly}
                                        onChange={(e) => !readOnly && updateBlock(block.id, 'notes', e.target.value)}
                                        placeholder="Опишите, как ответил кандидат, на чем запнулся, какие сильные стороны проявил..."
                                        className={`w-full rounded-xl border border-zinc-200 bg-white px-4 py-3 text-sm outline-none focus:border-violet-400 focus:ring-4 focus:ring-violet-100 resize-none transition-all placeholder:text-zinc-400 ${readOnly ? 'bg-zinc-50 cursor-default' : ''}`}
                                    />
                                </div>
                ...
                <div className="rounded-2xl border border-zinc-200 bg-zinc-900 p-8 shadow-md text-white mt-4 flex flex-col gap-6">
                    ...
                    <textarea
                        ...
                        value={overallNotes}
                        readOnly={readOnly}
                        onChange={(e) => !readOnly && setOverallNotes(e.target.value)}
                        placeholder="Как вам кандидат в целом? Насколько он мэтчится с командой?..."
                        className={`w-full rounded-xl border border-zinc-700 bg-zinc-800/50 px-4 py-3 text-sm text-white outline-none focus:border-violet-500 focus:ring-4 focus:ring-violet-500/20 resize-none transition-all placeholder:text-zinc-600 ${readOnly ? 'cursor-default' : ''}`}
                    />
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-3 gap-3">
                            <button
                                type="button"
                                disabled={readOnly}
                                onClick={() => !readOnly && setConfidence('HIGH')}
                                className={`rounded-xl border py-3 text-sm font-bold transition-all ${confidence === 'HIGH' ? 'bg-violet-600 border-violet-500 text-white' : 'bg-transparent border-zinc-700 text-zinc-300 hover:bg-zinc-800'} ${readOnly ? 'cursor-default' : ''}`}
                            >
                                Высокая (Все ясно)
                            </button>
                            <button
                                type="button"
                                disabled={readOnly}
                                onClick={() => !readOnly && setConfidence('MEDIUM')}
                                className={`rounded-xl border py-3 text-sm font-bold transition-all ${confidence === 'MEDIUM' ? 'bg-violet-600 border-violet-500 text-white' : 'bg-transparent border-zinc-700 text-zinc-300 hover:bg-zinc-800'} ${readOnly ? 'cursor-default' : ''}`}
                            >
                                Средняя
                            </button>
                            <button
                                type="button"
                                disabled={readOnly}
                                onClick={() => !readOnly && setConfidence('LOW')}
                                className={`rounded-xl border py-3 text-sm font-bold transition-all ${confidence === 'LOW' ? 'bg-violet-600 border-violet-500 text-white' : 'bg-transparent border-zinc-700 text-zinc-300 hover:bg-zinc-800'} ${readOnly ? 'cursor-default' : ''}`}
                            >
                                Низкая (Сомневаюсь)
                            </button>
                        </div>
                ...
                {!readOnly && (
                    <div className="sticky bottom-6 z-10 flex items-center justify-between mt-4 rounded-full border border-zinc-200 bg-white/90 backdrop-blur-md p-2 shadow-lg ring-1 ring-zinc-900/5 px-6">
                        ...
                        <button
                            type="submit"
                            disabled={!canSubmit}
                            className={`rounded-full px-8 py-3 text-sm font-bold shadow-sm transition-all focus:ring-4 focus:ring-violet-100 ${canSubmit ? 'bg-zinc-900 text-white hover:bg-black' : 'bg-zinc-100 text-zinc-400 cursor-not-allowed'
                                }`}
                        >
                            Отправить Протокол
                        </button>
