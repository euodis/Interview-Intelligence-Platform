...
export default function InterviewProtocol({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (plan && Object.keys(formState).length === 0) {
      ...
      plan.blocks.forEach(b => {
        initial[b.id] = { score: null, notes: "", signal: null };
      });
  ...
  // Validation
  const requiredBlocks = plan.blocks.filter(b => b.required);
  const completeRequired = requiredBlocks.every(b => formState[b.id]?.score !== null && formState[b.id]?.notes.trim().length > 5);
                    ...
                    <div className="bg-zinc-50/50 p-6 border-b border-zinc-100 flex flex-col gap-4">
                        ...
                        <div className="bg-white rounded-xl border border-zinc-200 p-4 shadow-sm">
                            ...
                            <ul className="text-sm text-zinc-700 list-disc list-inside space-y-1.5 marker:text-violet-400">
                                {block.questions.map((q, idx) => <li key={idx}>{q}</li>)}
                            </ul>
                        </div>
                    </div>
