...
import { mockCandidates, mockVacancies, mockInterviewers, mockBlockEvaluations, mockPlans, mockSessions } from "@/data/mocks";
...
export default function CandidateSummaryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [summary, setSummary] = useState<GeneratedCandidateSummary | null>(null);
  const [isGenerating, setIsGenerating] = useState(false);
  const [candidateInfo, setCandidateInfo] = useState<any>(null);

  // Payload for AI
  const [evaluationsPayload, setEvaluationsPayload] = useState<any[]>([]);
  ...
  useEffect(() => {
     ...
     setCandidateInfo({ candidate, vacancy, plan });

     const targetSessions = mockSessions.filter(s => s.candidateId === candidate.id && s.status === 'ЗАВЕРШЕНО');
     
     const evalsToSubmit: any[] = [];
     targetSessions.forEach(session => {
         const inter = mockInterviewers.find(i => i.id === session.interviewerId);
         const blocksEvals = mockBlockEvaluations.filter(e => e.sessionId === session.id);
         
         blocksEvals.forEach(ev => {
             const blockTitle = plan?.blocks.find(b => b.id === ev.blockId)?.title || "Unknown Block";
             evalsToSubmit.push({
     ...
     setEvaluationsPayload(evalsToSubmit);
  }, [candidateId]);
      ...
      <div className="flex flex-col gap-3 border-b border-zinc-200 pb-6 mt-2">
         ...
         <div className="flex items-center justify-between">
            <div>
               ...
               <p className="text-zinc-500 mt-1">Кандидат: <span className="font-semibold text-zinc-700">{candidateInfo.candidate.name}</span> • Вакансия: {candidateInfo.vacancy.title}</p>
            </div>
            
            <button 
                onClick={handleGenerate}
                disabled={isGenerating || evaluationsPayload.length === 0}
                className={`flex items-center gap-2 rounded-xl px-5 py-2.5 font-bold shadow-sm transition-all focus:ring-4 focus:ring-violet-100 ${
                    isGenerating ? 'bg-violet-400 text-white cursor-not-allowed' : 'bg-violet-600 hover:bg-violet-700 text-white'
                }`}
            >
               {isGenerating ? (
                   <>
      ...
      {summary && !isGenerating && (
          <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
             ...
             <div className="rounded-2xl border border-zinc-900 bg-zinc-900 p-6 flex items-center justify-between text-white mt-2">
                ...
                <button className="px-6 py-2.5 bg-white text-zinc-900 rounded-xl font-bold hover:bg-zinc-100 transition-colors">
                    Утвердить решение
                </button>
             </div>
          </div>
