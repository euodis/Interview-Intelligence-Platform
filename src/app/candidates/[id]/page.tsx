  ...
  return (
    <div className="flex flex-col gap-6 max-w-6xl mx-auto w-full pb-20">
      ...
      <div className="flex flex-col gap-4 border-b border-zinc-200 pb-6 mt-2">
        ...
        <div className="flex items-start justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="flex items-center gap-2 rounded-xl bg-violet-50 text-violet-700 px-4 py-2 font-medium shadow-sm hover:bg-violet-100 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm flex flex-col justify-between">
              <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-widest mb-4">Интервьюеры</h3>
              <div className="flex -space-x-3">
                  {assignedInterviewers.map((int: any, idx) => (
                      <div key={idx} title={`${int.name} - ${int.role}`} className="w-12 h-12 rounded-full border-[3px] border-white bg-zinc-200 flex items-center justify-center font-bold text-zinc-600 shadow-sm z-10 transition-transform hover:z-20 hover:-translate-y-1 cursor-pointer">
                          {int.name.charAt(0)}
                      </div>
                  ))}
                  {assignedInterviewers.length === 0 && <span className="text-sm text-zinc-400">Не назначены</span>}
              </div>
          </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 mt-2 items-start">
         ...
         <div className="w-full lg:w-2/3 flex flex-col gap-4">
             <div className="rounded-2xl border border-zinc-200 bg-white shadow-sm overflow-hidden">
                ...
                {plan && completedSessions.length > 0 ? (
                    ...
                    <div className="p-10 flex flex-col items-center justify-center text-center">
                        ...
                        <h4 className="font-semibold text-zinc-900">Интервью еще не проведены</h4>
                        <p className="text-zinc-500 text-sm mt-1 max-w-sm">Как только эксперты заполнят протоколы, здесь появится сводная матрица оценок.</p>
                    </div>
                )}
             </div>
         </div>
